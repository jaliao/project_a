...
- [ ] 6.2 〔待實機 UI〕端到端：維護範本 → 老師結業勾選學員 → 結業學員收到結業信（測試帳號集中至 justin@blockcode.com.tw），變數正確替換
  - ⚠️ 2026-06-22 實機測試發現**沒收到信**：根因為當初 apply 時 `graduateCourse`（`app/actions/course-invite.ts`）的寄信區塊**未實際寫入**（屬被偽造、未落地的編輯）。已補上寄信區塊並改為 `await`（結業 commit 後寄送，確保送出、失敗不影響結業）。SMTP（Brevo）實寄驗證 `SENT_OK`。待使用者重新以新課程實機確認。
- [ ] 6.3 〔待實機 UI〕邊界：未勾選學員不寄；未設定範本用預設；寄信失敗不影響結業；未知變數原樣保留
