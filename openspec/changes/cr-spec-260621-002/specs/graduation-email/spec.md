...
系統 SHALL 於 `/admin/settings` 提供結業信範本維護（主旨與內文），存於 `AdminSetting`（key `graduation_email_subject`、`graduation_email_body`），僅 superadmin 可編輯。未設定時 SHALL 使用系統預設範本。

#### Scenario: 維護結業信範本
- **WHEN** superadmin 於 `/admin/settings` 編輯結業信主旨與內文並儲存
...
老師執行 `graduateCourse` 確認結業後，系統 SHALL 對本次結業（`graduatedAt` 設值）的每位學員寄送結業信；收件地址依 `resolveContactEmail` 規則決定。寄送為 fire-and-forget，SHALL NOT 阻塞或影響結業流程；寄送失敗僅記錄錯誤。未通過結業的學員 SHALL NOT 收到結業信。
