...
- [ ] 10.2 確認 `.env` 已填入真實 R2 憑證與 `R2_PUBLIC_URL`（前置條件，非本次程式碼可驗證項目，若未填妥則以下手動驗證項目無法進行，需等使用者完成環境設定）
- [ ] 10.3 於個人資料頁上傳一張 jpg 圖片，確認上傳成功、頭像立即顯示、R2 bucket 內出現對應物件
- [ ] 10.4 上傳 gif 或超過 2MB 的圖片，確認被拒絕並顯示對應錯誤提示
- [ ] 10.5 更換頭像（上傳第二張圖片），確認新頭像生效、R2 bucket 內舊物件已被刪除
- [ ] 10.6 點擊「移除頭像」，確認 `avatarKey` 清空、R2 物件被刪除、頁面 fallback 顯示正確（有 Google 頭像者顯示 Google 頭像，無則顯示預設圖示）
- [ ] 10.7 確認學員專屬頁面、Topbar、站內訊息三處皆正確顯示頭像（含 fallback 情境：無自訂頭像的其他測試帳號應顯示預設圖示，不應破圖或顯示他人頭像）
