...
「我的學習」的書籍子頁 SHALL 僅在該課程目錄「已解鎖」時允許檢視與撰寫分段查經。某課程目錄對某使用者 SHALL 視為已解鎖，當且僅當該使用者在該 `courseCatalogId` 下存在任一報名，其 `status` 為 `approved`、未取消，且所屬開課的 `startedAt` 已設定（`completedAt` 已設定者亦成立）。解鎖判定 SHALL 為即時計算，SHALL NOT 因課程結業或取消單一開課而失效（只要仍存在符合條件的其他報名）。書籍選擇頁的三張書籍卡片 SHALL 皆顯示，未解鎖或設定檔無大綱者 SHALL 以鎖定樣式呈現、SHALL NOT 連入子頁。已解鎖但存取子頁時，若目錄無大綱亦比照「非法的 catalogId」重定向。「我的學習」筆記 SHALL NOT 綁定任一特定 `CourseInvite` 或報名。

#### Scenario: 已解鎖書籍卡片可進入

- **WHEN** 使用者在「啟動靈人」目錄有一筆 `approved`、未取消、所屬開課 `startedAt` 非空的報名，且設定檔有該目錄大綱
- **THEN** 書籍選擇頁的「啟動靈人」卡片為可點擊狀態，點擊導向 `/user/{spiritId}/learning/1`

#### Scenario: 未解鎖書籍卡片鎖定

- **WHEN** 使用者在某目錄無任何已開始的報名
- **THEN** 該書籍卡片以鎖定樣式顯示、不可點擊、顯示鎖定提示（尚未開課）

#### Scenario: 尚無大綱的書籍卡片鎖定

- **WHEN** 某書籍（如啟動豐盛）在設定檔中尚無課次大綱
...
課程大綱（課程目錄 → 課次 → 經文項目）SHALL 由程式碼設定檔 `config/learning-outline.ts` 定義，作為單一事實來源。每個課次 SHALL 具備穩定識別 `lessonKey`、顯示標題與經文項目清單；每個經文項目 SHALL 具備穩定識別 `scriptureKey` 與顯示標籤。`lessonKey` 與 `scriptureKey` 一經發布 SHALL 視為不可變（筆記以其參照）；顯示文字（`title` / `label`）可調整。「啟動靈人」（`courseCatalogId = 1`）之大綱 SHALL 包含十二課，`lessonKey` 為 `lesson-01` ~ `lesson-12`，其中 `lesson-01`（第一課：接受禮物）與 `lesson-12`（第十二課：靈人全開啟）無經文項目，其餘十課各含三個經文項目（依需求單所列書卷章次）。既有 `lesson-01`／`lesson-02` 之 `key` SHALL 保留不變，僅更新其 `title`（含第二課「裡」→「禮」之更正）。其他課程目錄之大綱 MAY 為空，待日後於設定檔追加。
...
- **WHEN** 使用者展開「第一課：接受禮物」或「第十二課：靈人全開啟」的課次卡片
- **THEN** 展開區塊顯示「本課次無分段查經」之提示，不提供任何筆記填寫入口

#### Scenario: 既有筆記不受標題更新影響

- **WHEN** 使用者在改版前已於 `lesson-01` 或 `lesson-02` 撰寫過筆記
- **THEN** 改版後這些筆記仍歸屬於對應課次（第一課／第二課），內容完整
