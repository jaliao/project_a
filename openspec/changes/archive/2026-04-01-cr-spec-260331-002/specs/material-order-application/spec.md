...
#### Scenario: 選擇 7-11 取貨時顯示 ECPay 門市選擇器
- **WHEN** 使用者選擇 `DeliveryMethod.sevenEleven`
- **THEN** 隱藏 `deliveryAddress` 文字輸入欄位，顯示 `EcpayStoreSelector`（`logisticsSubType="UNIMART"`）

#### Scenario: 選擇全家取貨時顯示 ECPay 門市選擇器
- **WHEN** 使用者選擇 `DeliveryMethod.familyMart`
- **THEN** 隱藏 `deliveryAddress` 文字輸入欄位，顯示 `EcpayStoreSelector`（`logisticsSubType="FAMI"`）

#### Scenario: 選擇郵寄時顯示文字欄位
- **WHEN** 使用者選擇 `DeliveryMethod.delivery`
- **THEN** 顯示 `deliveryAddress` 文字輸入欄位，隱藏門市選擇器，`storeId` / `storeName` 維持空值

#### Scenario: 已選取超商門市後顯示門市資訊
- **WHEN** `deliveryMethod` 為 `sevenEleven` 或 `familyMart` 且已選取門市
- **THEN** `EcpayStoreSelector` 顯示「已選取：{storeName}（{storeId}）」及「重新選取」按鈕
...
- **WHEN** 講師點擊「查看教材申請」
- **THEN** 表單所有欄位預填現有 CourseOrder 資料，若已選超商門市則 `EcpayStoreSelector` 顯示已選取狀態（`storeName（storeId）`）

#### Scenario: 無關聯 CourseOrder 時開啟表單
- **WHEN** 講師點擊「申請教材」（首次）
- **THEN** 表單以講師 Profile 及 CourseInvite.courseDate 預填，`storeId` 與 `storeName` 為空
...
#### Scenario: 選擇超商但未選取門市
- **WHEN** 使用者選擇 7-11 或全家取貨但未選取門市即送出
