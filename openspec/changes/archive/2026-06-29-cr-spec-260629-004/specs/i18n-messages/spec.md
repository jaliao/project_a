...
系統 SHALL 以 `messages/<locale>.json` 管理 UI 文案，`messages/zh-TW.json`（繁體）為**唯一事實來源**，`messages/en.json` 為英文翻譯，`messages/zh-CN.json` 為產生物。訊息 SHALL 以命名空間（如 `common`、`auth`、`nav`）組織。

#### Scenario: 以 key 取用文案
- **WHEN** 元件需要顯示文案
- **THEN** 透過翻譯 key（如 `auth.loginButton`）取用，不寫死語言字串
