...
系統 SHALL 以 `messages/<locale>.json` 管理 UI 文案，`messages/zh-TW.json`（繁體）為**唯一事實來源**，`messages/en.json` 為英文翻譯，`messages/zh-CN.json` 為產生物。訊息 SHALL 以命名空間組織，至少包含 `common`、`nav`、`auth`、`language`、`validation`、`status`、`role`、`catalog`，以及 feature 命名空間（如 `account`、`onboarding`、`recover`、`notifications`、`invites`、`learning`、`matchBoard`）。

#### Scenario: 以 key 取用文案
- **WHEN** 元件需要顯示文案
- **THEN** 透過翻譯 key（如 `auth.loginButton`、`validation.realNameRequired`、`status.recruiting`、`recover.title`）取用，不寫死語言字串
...
- **WHEN** 需要共用詞（動作/導覽/驗證/標籤）或 feature 頁文案
- **THEN** 分別取用共用命名空間（`common`/`nav`/`validation`/`status`/`role`/`catalog`）或對應 feature 命名空間（`account`/`onboarding`/`recover`/…）
