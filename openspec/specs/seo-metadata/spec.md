...
## Purpose
全站 SEO 技術基礎建設：站台對外網址單一來源（getSiteUrl）、根 metadata（metadataBase／預設 title／description／keywords／OpenGraph／Twitter／canonical／hreflang／robots 指示）、app/robots.ts 與 app/sitemap.ts 產生、以及可重用的 JSON-LD 結構化資料輸出（Organization／WebSite）。對外可索引的行銷內容頁見 public-course-intro。
