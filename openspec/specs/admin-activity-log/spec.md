# admin-activity-log Specification

## Purpose
後台「系統活動紀錄」檢視面：管理者於 `/admin/activity-logs` 總覽全系統的 `AdminActionLog` 管理操作紀錄（快照欄呈現、每頁 30 筆、動作類型／關鍵字／日期區間三種可組合篩選），並於 `/admin/activity-logs/rules` 查看自動依 `config/admin-log-action.ts` 產生的記錄規則說明。寫入面（哪些操作會產生紀錄）見 `admin-operation-log`。
