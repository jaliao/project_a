...
每位會員的身分集合 SHALL 至少包含 `user`（一般會員為基底）；`teacher_1`、`teacher_2`、`teacher_3`、`teacher_4`、`admin`、`superadmin` 為加掛身分。系統 SHALL NOT 允許將身分集合清空至不含 `user`。
...
- `canTeachAny`：身分集合含任一書籍講師身分（`teacher_1`～`teacher_4`），或含 `admin` 或 `superadmin`；用於判斷是否顯示講師相關入口或視圖
...
- **WHEN** 系統需判斷 `courseCatalogId = 4` 的授課所需身分
- **THEN** 取得 `teacher_4`（啟動事工 4 講師）
